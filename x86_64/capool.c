...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 9, 4);
 	...
 	ivoc_help("help ?1 capool /usr/local/home/akozlov/prog/tectum/x86_64/capool.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
 static double FARADAY = 96489;
static int _reset;
static char *modelname = "N-type calcium pool, lamprey spinal interneuron";
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   ...
   Dcai = drive_channel + ( cainf - cai ) / catau ;
   }
 return _reset;
}
